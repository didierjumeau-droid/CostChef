...
- **Recipe Profitability Dashboard**
  - Dedicated form (`RecipeProfitabilityForm`) to rank recipes by profit margin and food cost %.
  - Shows cost per serving, selling price, gross profit and margin %.
  - Color-coded rows to highlight very profitable vs weak items.

- **Purchasing (automatic stock & price updates)**
  - Record supplier deliveries (date, supplier, invoice, ingredient lines).
  - Automatically updates inventory stock levels for each purchased ingredient.
  - Optionally updates ingredient unit prices according to supplier rules and logs changes in `price_history`.
